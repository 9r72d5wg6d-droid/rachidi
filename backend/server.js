...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (method === 'POST' && route === '/api/reservations') { const input = await body(req); const professor = findPerson(input.professorId); const room = db.rooms.find(r => r.id === input.roomId); if (!professor || professor.type !== 'professor' || !room) return error(res, 400, 'Professeur ou salle invalide.'); if (!input.date || !input.startTime || !input.endTime || input.endTime <= input.startTime) return error(res, 400, 'La période de réservation est invalide.'); const conflict = db.reservations.find(r => r.roomId === input.roomId && r.date === input.date && r.status !== 'cancelled' && input.startTime < r.endTime && input.endTime > r.startTime); if (conflict) return error(res, 409, 'Conflit : cette salle est déjà réservée sur ce créneau.'); const reservation = { id: id(), roomId: room.id, professorId: professor.id, date: input.date, startTime: input.startTime, endTime: input.endTime, purpose: String(input.purpose || '').trim(), status: 'confirmed', createdAt: now() }; db.reservations.push(reservation); save(db); return respond(res, 201, { success: true, data: reservation }); }
    if (method === 'PATCH' && /^\/api\/reservations\/[^/]+$/.test(route)) { const reservation = db.reservations.find(r => r.id === route.split('/').pop()); const input = await body(req); if (!reservation) return error(res, 404, 'Réservation introuvable.'); if (!['confirmed','cancelled','completed'].includes(input.status)) return error(res, 400, 'Statut invalide.'); reservation.status = input.status; save(db); return respond(res, 200, { success: true, data: reservation }); }
